...
(function () {
  ...
  style.textContent = [
    '@keyframes cbPulse{0%{transform:scale(1);opacity:.7}70%{transform:scale(1.55);opacity:0}100%{transform:scale(1.55);opacity:0}}',
    '.cb-toggle{position:fixed;bottom:28px;right:28px;width:64px;height:64px;border-radius:50%;background:linear-gradient(135deg,#38B6FF 0%,#0B1F5E 100%);color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 18px rgba(56,182,255,.5),0 8px 32px rgba(11,31,94,.45);z-index:1100;transition:transform .25s ease,box-shadow .25s ease;}',
    '.cb-toggle:hover{transform:scale(1.1);box-shadow:0 6px 28px rgba(56,182,255,.7),0 10px 40px rgba(11,31,94,.5);}',
    '.cb-toggle::before{content:"";position:absolute;inset:0;border-radius:50%;border:3px solid #38B6FF;animation:cbPulse 2.4s ease-out infinite;pointer-events:none;}',
    '.cb-toggle.open::before{animation:none;opacity:0;}',
    '.cb-toggle svg{transition:opacity .2s ease,transform .2s ease;position:absolute;}',
    '.cb-toggle.open svg.icon-chat{opacity:0;transform:scale(.7);}',
    '.cb-toggle:not(.open) svg.icon-chat{opacity:1;transform:scale(1);}',
    '.cb-toggle.open svg.icon-close{opacity:1;transform:scale(1);}',
    '.cb-toggle:not(.open) svg.icon-close{opacity:0;transform:scale(.7);}',
    '.cb-badge{position:absolute;top:-2px;right:-2px;width:20px;height:20px;background:#e53e3e;border-radius:50%;font-size:10px;font-weight:700;color:#fff;display:flex;align-items:center;justify-content:center;opacity:0;transform:scale(0);transition:opacity .2s,transform .2s;border:2px solid #fff;}',
    '.cb-badge.show{opacity:1;transform:scale(1);}',
    '.cb-backdrop{position:fixed;inset:0;background:rgba(8,15,46,.35);z-index:1099;opacity:0;transition:opacity .3s ease;pointer-events:none;}',
    '.cb-backdrop.open{opacity:1;pointer-events:auto;}',
    '.cb-window{position:fixed;top:50%;left:50%;transform:translate(-50%,-46%) scale(.97);width:540px;max-width:calc(100vw - 24px);height:740px;max-height:calc(100vh - 60px);background:#fff;border-radius:20px;box-shadow:0 24px 80px rgba(11,31,94,.3);display:flex;flex-direction:column;overflow:hidden;z-index:1100;opacity:0;transition:opacity .3s ease,transform .3s ease;pointer-events:none;}',
    '.cb-window.open{opacity:1;transform:translate(-50%,-50%) scale(1);pointer-events:auto;}',
    '.cb-header{padding:24px;background:linear-gradient(135deg,#0B1F5E 0%,#114CBF 100%);color:#fff;display:flex;align-items:center;gap:12px;flex-shrink:0;}',
    '.cb-avatar{width:62px;height:62px;border-radius:50%;background:rgba(56,182,255,.18);display:flex;align-items:center;justify-content:center;flex-shrink:0;overflow:hidden;border:2px solid rgba(255,255,255,.25);}',
    ...
    '.cb-header-role{font-size:.68rem;font-weight:600;letter-spacing:.08em;text-transform:uppercase;opacity:.6;margin:1px 0 3px;}',
    '.cb-header-status{font-size:.74rem;opacity:.85;display:flex;align-items:center;gap:5px;}',
    ...
    '.cb-messages{flex:1;overflow-y:auto;padding:16px;display:flex;flex-direction:column;gap:10px;scroll-behavior:smooth;background:#eef1f7;}',
    '.cb-messages::-webkit-scrollbar{width:4px;}',
    '.cb-messages::-webkit-scrollbar-thumb{background:rgba(17,76,191,.18);border-radius:4px;}',
    '.cb-msg{max-width:88%;line-height:1.5;font-size:.95rem;animation:cbFadeUp .25s ease forwards;}',
    ...
    '.cb-bubble{padding:14px 18px;border-radius:16px;white-space:pre-wrap;word-break:break-word;}',
    '.cb-msg.bot .cb-bubble{background:#fff;color:#0B1F5E;border-bottom-left-radius:4px;box-shadow:0 2px 10px rgba(0,0,0,.07);}',
    ...
    '.cb-option{background:#fff;border:none;box-shadow:0 2px 10px rgba(0,0,0,.07);color:#0B1F5E;border-radius:12px;padding:13px 18px;font-size:.84rem;font-weight:500;text-align:left;cursor:pointer;transition:box-shadow .2s,background .2s;line-height:1.4;}',
    '.cb-option:hover,.cb-option:active{box-shadow:0 4px 18px rgba(56,182,255,.22);background:rgba(56,182,255,.04);}',
    '.cb-typing{display:flex;align-items:center;gap:4px;padding:12px 14px;background:#fff;border-radius:16px;border-bottom-left-radius:4px;width:fit-content;box-shadow:0 2px 10px rgba(0,0,0,.07);}',
    ...
    '.cb-text-input{flex:1;padding:13px 18px;border:1.5px solid rgba(17,76,191,.12);border-radius:12px;font-size:.875rem;color:#0B1F5E;outline:none;background:#f4f6fa;transition:border-color .2s;}',
    '.cb-text-input:focus{border-color:#38B6FF;}',
    '.cb-text-input::placeholder{color:#a0aec0;}',
    '.cb-send-btn{width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,#38B6FF,#114CBF);color:#fff;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:opacity .2s;}',
  ...
  win.innerHTML = [
    '<div class="cb-header">',
    '  <div class="cb-avatar"><img src="mobiwebbot.webp" alt="Mobiweb Assistant" /></div>',
    '  <div class="cb-header-info">',
    '    <div class="cb-header-name">Mobiweb Assistant</div>',
    '    <div class="cb-header-role">AI Business Advisor</div>',
    '    <div class="cb-header-status"><div class="cb-dot"></div>Online &amp; ready to help</div>',
    '  </div>',
